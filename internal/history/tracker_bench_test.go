...
func BenchmarkGetTrend(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := tracker.GetTrend(ctx, WithTrendBranch(DefaultBranch))
		if err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkGetLatestEntry(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := tracker.GetLatestEntry(ctx, DefaultBranch)
		if err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkConcurrentRecord(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			...
			if err := tracker.Record(ctx, coverage, WithBranch(DefaultBranch), WithCommit(commit, "")); err != nil {
